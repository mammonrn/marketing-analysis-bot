...
function buildUserContent({ question, dataContext, turns, isSummary }) {
  ...
  if (dataContext) {
    parts.push(
      '--- ข้อมูลจริงจาก Google Sheets (read-only) ---\n' +
        'ตัวเลขด้านล่างนี้เป็นค่าที่อ่านมาจากไฟล์จริง ใช้เฉพาะข้อมูลนี้ ห้ามเดาตัวเลขเพิ่ม\n' +
        dataContext,
    );
  } else if (!isSummary) {
    parts.push(
      '--- ไม่มีข้อมูลจาก Sheets สำหรับคำถามนี้ ---\n' +
        'ยังไม่ได้เชื่อมข้อมูล หรือไม่พบแท็บที่เกี่ยวข้อง — ห้ามแต่งตัวเลขขึ้นมาเอง ' +
        'ให้บอกผู้ใช้ว่าต้องใช้ข้อมูลส่วนไหน และใส่เหตุผลใน data_gaps',
    );
  }
