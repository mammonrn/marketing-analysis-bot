...
## PIN ของ Dashboard สรุปเดือน (ต้องตั้ง ไม่งั้นเปิดหน้า dashboard ไม่ได้)

ลิงก์สรุปเดือนถูกพิมพ์เป็นข้อความไว้ในแชทด้วย เพราะ Telegram **ตัดปุ่ม `web_app` ทิ้ง
เวลามีคน forward ข้อความต่อ** — คนที่รับต่อจะเหลือแค่ URL ในเนื้อข้อความเป็นทางเดียวที่เข้าได้
ลิงก์เลยอายุ 60 วันและใช้ซ้ำได้ ซึ่งแปลว่า token ไม่ใช่ความลับอีกแล้ว → ข้อมูลอยู่หลัง PIN

```bash
cd /var/www/ads-analytics-bot
npm run hash:pin              # พิมพ์ PIN 2 ครั้ง (ไม่ขึ้นบนจอ ไม่เข้า history)
# ได้บรรทัด DASHBOARD_PIN_HASH=scrypt$... → เอาไปใส่ .env
nano .env
pm2 restart ads-analytics-bot

npm run hash:pin -- --verify  # ตรวจว่า PIN ที่จำไว้ตรงกับ hash ใน .env
```

- PIN ตัวเดียวใช้ร่วมกันทุกเว็บ (SH666 / U89 / 88F) — ไม่แยกตามเว็บ
- เก็บเป็น scrypt hash เท่านั้น ไม่มีที่ไหนเก็บ PIN ตรง ๆ ทั้ง .env, SQLite และ log
- กรอกถูกครั้งเดียวจำไว้ 12 ชม. (`DASHBOARD_SESSION_HOURS`) ต่อเบราว์เซอร์
- กรอกผิดติดกัน 5 ครั้ง = ล็อก IP นั้น 15 นาที (`DASHBOARD_PIN_MAX_ATTEMPTS`,
  `DASHBOARD_PIN_LOCKOUT_MINUTES`) — ระหว่างล็อกกรอกถูกก็ยังไม่ให้เข้า
- **เปลี่ยน PIN** = สร้าง hash ใหม่ + restart → ทุกคนที่ปลดล็อกไว้ต้องกรอกใหม่หมด
- ถ้าไม่ตั้ง `DASHBOARD_PIN_HASH` หน้า dashboard จะตอบ 503 กับทุกคน (fail closed)
  ไม่ใช่เปิดข้อมูลทิ้งไว้ — ดู log `DASHBOARD_PIN_HASH is not set`
...
| ปุ่ม Dashboard กดแล้วขึ้น "ลิงก์หมดอายุ" | token สรุป session อายุ 1 ชม. — สั่ง `/สรุป` ใหม่ |
| ไม่มีปุ่ม Dashboard เลย | `PUBLIC_URL` ไม่ได้ตั้ง (ดู log `PUBLIC_URL not set`) |
| หน้าสรุปเดือนขึ้น "ยังไม่ได้ตั้งค่า PIN" | ยังไม่ใส่ `DASHBOARD_PIN_HASH` ใน `.env` → `npm run hash:pin` |
| ลิงก์สรุปเดือนขึ้น "ลิงก์หมดอายุ" | ลิงก์เกิน `MONTHLY_LINK_DAYS` (60 วัน) — สั่งสรุปเดือนนั้นใหม่ จะได้ลิงก์ใหม่ |
| กรอก PIN ถูกแต่ขึ้นว่ารอ N นาที | ก่อนหน้านั้นกรอกผิดจนถูกล็อก — รอให้ครบ หรือ `pm2 restart` (ตัวนับอยู่ใน memory) |
| forward ข้อความแล้วปุ่มหาย | Telegram เป็นแบบนั้นเอง — ใช้ URL บรรทัด `🔗 ลิงก์:` ในข้อความแทน |
| กราฟไม่ขึ้นในทุกคำถาม | ไม่ใช่บั๊ก — spec §6 บังคับให้เป็นแบบนี้ |
| ครบ 20 นาทีแล้วบอทไม่ถามว่าจะสรุปไหม | ไม่ใช่บั๊ก — พฤติกรรมใหม่คือ **ปิด session แล้วแจ้งเฉย ๆ** ไม่ถามและไม่สรุปให้เอง อยากได้สรุปพิมพ์ `/สรุป` |
| ครบ 20 นาทีแล้วไม่มีข้อความปิด session เลย | session นั้นไม่มีคำถามค้างอยู่ (ปิดไปแล้ว หรือยังไม่เคยถาม) — sweeper แจ้งเฉพาะ session ที่ยังมี turn ค้าง |
